        ...
        layoutNodes: function (data, max_instance) {
            ...
            function reduce_links() {
                ...
                for (let index = 0; index < links.length; ++index) {
                    ...
                    // instance, rtb, eni need to remove unnecessary links
                    // instance and eni only exists in "source" of a "Is..." relation
                    if ((rel.source.type === "i" || rel.source.type === "eni") && rel.target.type !== "rtb") {
                    ...
                    // remove link sg - eni
                    else if(rel.source.type !== "sg" || (rel.source.type === "sg" && rel.target.type !== "eni")) {
                ...
                for (let resourceId in temp_links) {
                    ...
                    // rtb or eni: instance -> subnet -> vpc
                ...
                if (block_members.hasOwnProperty(idx)) {
                    ...
                    if (block_members[idx].subnet) {
                        ...
                        for (let subnet_idx = 0; subnet_idx < subnet_len; ++subnet_idx) {
                            ...
                            if (subnet.fields && subnet.fields.eni) {
                                let eni_len = subnet.fields.eni.length;
                                let enis = subnet.fields.eni;
                                let theta_eni = theta_sub / eni_len / 2.0;
                                for (let eni_idx = 0; eni_idx < eni_len; ++eni_idx) {
                                    let eni = enis[eni_idx];
                                    if(!eni.x || !eni.y){
                                        eni.x = x + 1.75 * r * Math.cos(theta_sub * (subnet_idx - 0.25) + theta_eni * eni_idx);
                                        eni.y = y + 1.75 * r * Math.sin(theta_sub * (subnet_idx - 0.25) + theta_eni * eni_idx);
                                    }
                                }
                            }
