    ...
    let TopologyPanel = SimpleSplunkView.extend({
        ...
        createView() {
            ...
            this.elements = {
                ...
                list_element: $(".aws-topology-list"),
                chart_element: $(".aws-topology-chart"),
                overview_element: $("#chart-overview"),
                recommendation_element: $('.aws-topology-recomm'),
            };
        ...
        formatData(data) {
            ...
            data.forEach((datum) => {
                ...
                // some nodes may not have relationships (elb should have relationship with vpc)
                ...
                relationships.forEach((relationship) => {
                    ...
                    if (datum.resourceId.indexOf("eni-") === 0 && relationship_target.indexOf("i-") === 0){
                        if(nodesForRelationships[relationship_target]){
                            nodesForRelationships[relationship_target].enis.push(datum.resourceId);
                        }
                    }
                });
            });
        ...
        _drawVpcScopes() {
            ...
            for(let i=0; i<nodes.length; i++){
                ...
                if(node.id.indexOf("vpc-") === 0){
                    ...
                    // set vpc region
                    vpcScopes[vpcId].region = node.awsRegion;
                }
            }
            ...
            for(let vpcId in vpcScopes){
                ...
                vpcLayer.append("rect").attr("ry", 10).attr("x", scope.xMin-padding).attr("y", scope.yMin-padding*1.2).attr("width", scope.xMax-scope.xMin+padding*2).attr("height", scope.yMax-scope.yMin+padding*2.2).style({"stroke": "#fd9805", "stroke-width":2, "stroke-opacity":0.6, "fill": "#FFFFFF", "opacity": 0.6});
                ...
                vpcLayer.append("text").attr("x", scope.xMin-padding+70).attr("y", scope.yMin-padding*1.2+25).text(scope.name).style({"font-size":"16px", "font-weight":"bold", "fill": "#fd9805", "opacity": 0.6});
                vpcLayer.append("text").attr("x", scope.xMin-padding+70).attr("y", scope.yMin-padding*1.2+45).text(scope.region).style({"font-size":"14px", "fill": "#fd9805", "opacity": 0.6});
            }
