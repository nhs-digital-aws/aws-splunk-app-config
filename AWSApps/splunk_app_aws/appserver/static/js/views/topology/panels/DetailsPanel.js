define([
    ...
    let DetailsPanel = SimpleSplunkView.extend({
        ...
        initialize(params) {
            ...
            this.briefTemplate = '\
                <table> \
                    ...
                    <% } %> \
                    <% if(node.type !== "policy"){ %> \
                    <tr><th>Account ID: </th><td><%-node.awsAccountId%></td></tr> \
                    <% if(node.type !== "user" && node.type !== "group"){ %> \
                    <tr><th>Region: </th><td><%-node.awsRegion%></td></tr> \
                    <% } %> \
                    <% } %> \
                    <% if (node.state) { %> \
            ...
            if (vpcFlowQueryTpl) {
                if (prefix === 'eni'){
                    ...
                    let node = this.config.nodes[id],
                        eniSearch = '';
                    if(node && node.enis.length > 0){
                        for(let i=0; i<node.enis.length; i++){
                            let eni = node.enis[i];
                            eniSearch += `OR interface_id="${eni}" `;
                        }
                        eniSearch = eniSearch.substr(2);
                        let vpcFlowQuery = vpcFlowQueryTpl.format(eniSearch);
                        tokenModel.set('vpcFlowQuery', vpcFlowQuery);
                    }
                }
        ...
        updateView(viz, data) {
            ...
            let drilldownTitle = recommendation ? `${recommendation.get('ml_action')} ${recommendation.get('resource_id')}` : id;
        ...
        _relocate() {
            let $chart = $('.aws-topology-chart');
            let $svg = $('.aws-topology-chart svg');
